<template>
  <div class="udss-board-body">
    <div class="scheme-header">
      名称&nbsp;&nbsp;<input style="width: 325px;" class="scheme-input" type="text" placeholder="评价方案名称" v-model="detailEvaluation.name">
      &nbsp;&nbsp;简称&nbsp;&nbsp;<input :class="['scheme-input', {'err-msg': lenError}]" type="text" placeholder="不超过8字符" v-model="detailEvaluation.short_name">
      <Button v-if="!nowEvaluation.is_published" v-hasPermission="'w'" type="primary" size="small" class="button-confirm" @click="confirmCreate" :loading="saveLoading">保存</Button>
    ...
    <div class="scheme-remark">
      ...
      <Input
        ...
        :rows="2"
        placeholder="备注信息"
        v-model="detailEvaluation.remark" />
    </div>
        ...
        <li v-for="node in allWeights" :key="node.id">
          ...
          <div class="scheme-main-right">
            <div v-for="(item, index) in node.orgs" :key="item.id" class="scheme-label scheme-label-card">
              {{item.name}}
              <i class="el-icon-circle-close-outline" @click="deleteCollege(node, item, index)" v-if="$route.meta.permissionList.includes('w') && !nowEvaluation.is_published"></i>
            </div>
            <Popover
              placement="right"
              width="160"
              ...
              <div slot="reference" class="scheme-label scheme-label-add"  v-if="$route.meta.permissionList.includes('w') && !nowEvaluation.is_published">+ 新增</div>
            </Popover>
          </div>
        </li>
      </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('evaluation', ['updateEvaluation']),
      ...mapMutations('evaluation', ['setEvaluationById']),
      ...mapMutations('config', ['exportDataA', 'exportDataB']),
      downloadDataA () {
        this.exportDataA({'name': '评价方案-' + this.detailEvaluation.name, 'url': '/v1/eva/exp/' + this.detailEvaluation.id})
      },
      downloadDataB () {
      ...
      // 验证字符串长度
      lenValid () {
        this.detailEvaluation.short_name.length < 9 ? this.lenError = false : this.lenError = true
      },
      confirmCreate () {
        this.lenValid()
        if (!this.lenError) {
          ...
          this.detailEvaluation.detail = detail
          this.detailEvaluation.order_no = this.nowEvaluation.order_no
          this.detailEvaluation.univ_id = 2
          this.detailEvaluation.user_id = 1
          new Promise((resolve, reject) => {
            this.saveLoading = true
            this.updateEvaluation({resolve, reject, payload: this.detailEvaluation})
          }).then(() => {
    ...
    created () {
      this.setEvaluationById([])
    },
    computed: {
      ...mapGetters('evaluation', ['colleges', 'detailEvaluation', 'nowEvaluation']),
      allWeights () {
    ...
    watch: {
      ...
      detailEvaluation () {
        if (this.detailEvaluation && this.detailEvaluation.weight_detail) {
          ...
          let detail = this.detailEvaluation.weight_detail
          detail.forEach(node => {
            node.orgs.length > 0 && node.orgs.forEach(item => {
              let index = this.newColleges.findIndex(child => child.id === item.id)
              index > -1 && this.newColleges.splice(index, 1)
            })
          })
        }
      }
