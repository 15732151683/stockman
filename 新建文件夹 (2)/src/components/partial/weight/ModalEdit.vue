  ...
  <div class="modal-bg">
    <div class="modal-content" @click.stop>
      <div class="modal-head">
        <input v-model="nowWeight.name" class="weight-name" />
        <div class="mySwitch" v-if="nowWeightInfo.login_is_created && nowWeightInfo.login_is_created">
          <div class="mySwitch-cover" @click="handleChangeSwitch(nowWeightInfo)"></div>
          <Switchs
            style="display: block"
            v-model="nowWeightInfo.is_published"
            active-color="#1EBBA6"
        ...
        <Button v-if="nowWeightInfo.login_is_created && !nowWeightInfo.is_published" type="primary" size="small" class="button-confirm" @click="confirmCreate" v-hasPermission="'w'" :loading="saveLoading" style="float: right;">
          {{$t('weight.confirm')}}
        </Button>
      </div>
      ...
      <div class="weight-check">
        <div class="weight-check-left">
          全局年份选择：<PickerDate :range="globalYear" @rangeChange="globalYearChange" :width="140" :size="'small'"></PickerDate>
        </div>
        <div class="weight-check-right">
          备注： <Input type="textarea" rows="1" v-model="nowWeight.remark" style="width: 90%;" />
        </div>
      </div>
...
<script>
  ...
  import {Message, Checkbox, Button, Input, Switch, MessageBox} from 'element-ui'
  import { PickerDate } from '../../widgets'
  export default {
    name: 'ModalEdit',
    mixins: [ TogglePage, ObjInstance, StandTree ],
    components: {
      Message, Checkbox, Button, Input, PickerDate, Switchs: Switch, MessageBox
    },
    data () {
      return {
        chart: {},
    ...
    mounted () {
      let timer = setInterval(() => {
        if (this.nowWeight && this.nowWeight.details.length > 0) {
          ...
          this.nowTableData.forEach(node => {
            this.nowWeight.infos.forEach(item => {
              if (item.id === node.id) {
                node.select = item.count
              }
            })
          })
      ...
      // console.log(this.nowWeightInfo, 'nowWeightInfo')
    },
    // 组件消失之前，清空当前方案数据，防止下次打开修改页面数据还保留
    beforeDestroy () {
      this.getNowWeight('')
    },
    methods: {
      ...mapActions('weight', ['updateWeight', 'getAllWeight', 'weightSwicth']),
      ...mapMutations('weight', ['getNowWeight']),
      handleChangeSwitch (row) {
        if (row.is_use) {
          Message({
            message: '该权重方案被评价方案引用，不能更改发布状态！',
            type: 'warning'
          })
        } else {
          if (row.is_published) {
            MessageBox.confirm('取消发布后其他人不可见，确定取消发布？', '提示', {
              ...
              new Promise((resolve, reject) => {
                this.weightSwicth({resolve, reject, id: row.id})
              }).then(() => {
                Message({
                  message: '取消发布成功！',
                  type: 'success'
      ...
      initValue () {
        ...
        let timer = now + '-' + now
        let results = this.clone(this.nowWeight.details)
        this.allfNodes.forEach((data) => {
          let result = results.find(item => item.id === data.id) || {'status': true}
          this.$set(data, 'weight', result.weight)
          this.$set(data, 'status', !result.status)
          data.year_range = result.year_range ? result.year_range : timer
        })
      },
      confirmCreate () {
        ...
        new Promise((resolve, reject) => {
          ...
          this.updateWeight({
            payload: { 'weightId': this.nowWeight.id, 'weightName': this.nowWeight.name, 'indicators': this.indicators, 'remark': this.nowWeight.remark },
            resolve,
            reject
          })
        }).then(() => {
          Message({
            message: this.$t('element.editSuccess'),
            type: 'success'
          })
          ...
          this.saveLoading = false
          Message({
            message: this.$t('element.editFailure') + message,
            type: 'error'
          })
        })
      },
    ...
    computed: {
      ...
      ...mapGetters('weight', ['nowWeight', 'nowWeightInfo']),
      val () {
        return this.indType[this.activeIndex]
      },
      allfNodes () {
