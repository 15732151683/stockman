      ...
      <div class="task-container-box">
        <ul class="task-ul" v-if="showList">
          <template v-for="(item, index) in tasks">
            <TaskLi :key="index" :currentTask="currentTask" :item="item" @handleDetail="handleIndex(item)">
              <div slot="aside">
                <Button
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('task', ['searchCondition', 'targets', 'tasks'])
    },
    methods: {
      ...mapMutations('commitTask', ['setNodeTree', 'setCurrentTask']),
      // ...mapMutations('commitTask', ['setTask']),
      // 根据id过滤符合节点树结构
