          ...
          tooltip: {
            ...
            formatter: function (params) {
              ...
              if (data.child) {
                data.child.forEach(node => {
                  childData += '<div class="chart-tooltip" style="float: left;"><ul class="list list2"><li class="list-item weighted center">' +
                    ...
                    '<span>' + _self.thousandium(node.val) + '</span></li>'
                  if (node.details) {
                    childData += node.details.map(item => {
                      return '<li class="list-item">' +
                      '<span>' + item.indicator_name + '</span>' +
                      '<span>' + _self.thousandium(item.value) + '</span></li>'
                    }).join('') + '</ul></div>'
                  }
                })
              }
              return '<div class="chart-box"><div class="chart-tooltip">' +
                ...
                '<li class="list-item weighted">' +
                '<span>' + _self.$t('output.score') + '</span>' +
                '<span>' + _self.thousandium(data.val) + '</span>' +
                '<span>(' + data.rank + '/' + len + ')</span>' +
                '</li>' +
          ...
          series: [{
            ...
            markLine: {
              ...
              label: {
                normal: {
                  ...
                  position: 'end',
                  formatter (params) {
                    return _self.$t('outIn.text') + _self.thousandium(params.value)
                  }
                }
              },
              symbol: 'image://',
            ...
            label: {
              normal: {
                ...
                color: '#333',
                formatter (params) {
                  return _self.thousandium(Number(_self.tree[params.dataIndex].val))
                }
              }
            }
          }]
