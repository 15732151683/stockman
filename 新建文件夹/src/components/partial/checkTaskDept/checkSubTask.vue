...
<script>
  ...
  import { Container, LogList, Tabs, SideTree, InputData, DataByInd } from '../../widgets'
  export default {
    name: 'CheckTask',
    components: {
      Container, LogList, Tabs, SideTree, InputData, DataByInd, Input, Button, MessageBox, Message
    },
    ...
    created () {
      let id = this.currentTask.sub_tasks.ind_ids[0]
      this.radios = id
    },
    mounted () {
      this.$nextTick(function () {
        new Promise((resolve, reject) => {
          this.getTaskLog({resolve, reject, payload: {id: this.currentTask.id, subId: this.currentTask.sub_tasks.id}})
        }).then((res) => {
          this.taskRemark = res
        }).catch(() => {})
        if (this.currentTask.task_node.id < 5) {
          this.passVisible = !!this.currentTask.sub_tasks.dept_accepted_at
          this.yearRange = this.currentTask.year_range
          new Promise((resolve, reject) => {
            this.getSubTaskError({resolve, reject, payload: {id: this.currentTask.id, subId: this.currentTask.sub_tasks.id}})
          }).then(() => {
    ...
    methods: {
      ...
      ...mapActions('checkTask', ['rejectTask', 'passTask']),
      ...
      handleSave () {
        let payload = {
          ...
          subId: this.currentTask.sub_tasks.id,
          move: false,
      ...
      handlePass () {
        if (this.errorList.length > 0) {
          MessageBox.confirm('还有错误数据没有处理，确定审核通过？', this.$t('element.warming'), {
            ...
            new Promise((resolve, reject) => {
              this.passTask({resolve, reject, payload})
            }).then(() => {
              this.passVisible = true
              this.togglePage('TaskList')
            }).catch(() => {})
        ...
        if (errorInd) {
          this.tabData = data.orgs.map(node => {
            ...
            node.val.forEach(item => {
              let idx = errorInd.data.find(child => child.id === item.ind_data_id)
              if (idx) {
                this.$set(item, 'remark', true)
                this.$set(item, 'text', idx.remark)
              } else {
