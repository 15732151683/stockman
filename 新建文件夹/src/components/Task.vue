        ...
        <div class="task-container-left">
          <ul class="task-ul" v-if="showList">
            <template v-for="(item, index) in tasks">
              <TaskLi :key="index" :currentTask="currentTask" :item="item" @handleDetail="handleDetail(item)">
                <div slot="aside">
                  <div class="task-button-box" v-if="!item.publishedAt">
                  ...
                  <Button
                    v-if="item.publishedAt && !item.finishedAt && !item.terminatedAt"
                    type="primary"
                    size="mini"
                    class="button-add"
        ...
        <div class="task-form" v-if="addVisible">
          <div class="task-form-left">
            <Form
              ...
              <FormItem label="指标年份" prop="yearRange">
                <PickerDate
                  @rangeChange="yearChange"
                  :width="'100%'"
                  :height="'100%'"
                  :size="''"
                  :range="taskInfo.yearRange">
                </PickerDate>
              </FormItem>
              ...
              <FormItem label="填报类型" prop="typeAId">
                <Select v-model="taskInfo.typeAId" placeholder="请选择" style="width: 100%;" @change="changeTypeA">
                  <Option
                    v-for="item in typeA"
                    :key="item.id"
                    :label="item.name"
              ...
              <FormItem label="填报学校" prop="orgIds">
                <Select v-model="taskInfo.orgIds" placeholder="请选择" style="width: 100%;" multiple>
                  <Option
                    v-for="item in selectSchools"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </Option>
        ...
        <div class="task-form" v-if="updateVisible">
          <div class="task-form-left">
            <Form
              ...
              <FormItem label="填报类型" prop="typeAId">
                <Select v-model="taskInfo.typeAId" placeholder="请选择" style="width: 100%;" @change="changeTypeA" disabled>
                  <Option
                    v-for="item in typeA"
                    :key="item.id"
                    :label="item.name"
  ...
  export default {
    ...
    mounted () {
      ...
      let timer = setInterval(() => {
        if (this.targets.length > 0 && this.tasks.length > 0) {
          this.currentTask = this.tasks[0]
          this.getNodeTree(this.tasks[0])
          this.showList = true
          clearInterval(timer)
        }
      }, 10)
    ...
    computed: {
      ...mapGetters('task', ['typeA', 'schools', 'targets', 'tasks', 'pageInfo'])
    },
    methods: {
      ...mapActions('task', ['getTarget', 'createTask', 'publishTask', 'updateTask', 'deleteTask', 'endTask']),
      ...mapMutations('task', ['setPageInfo', 'setSearchCondition', 'setTask']),
      yearChange (data) {
        this.taskInfo.yearRange = data
        this.$refs.createForm.validateField('yearRange')
      },
      initSelect (typeA) {
        this.selectSchools = this.schools.filter(node => node.typeA.id === typeA)
        let id = this.typeA.find(type => type.id === typeA)
        id = id ? id.indRootId : ''
        this.selectTarget = this.targets.filter(node => node.id === id)
      },
      changeTypeA () {
        this.initSelect(this.taskInfo.typeAId)
      },
      updateYearChange (data) {
        this.taskInfo.yearRange = data
        this.$refs.updateForm.validateField('yearRange')
      },
      // 处理节点树结构
      clone (data) {
      ...
      getNodeTree (nodes) {
        this.initSelect(nodes.typeAId)
        let cloneTree = this.clone(this.selectTarget)
        this.nodeTree = this.filterTree(cloneTree, nodes.indIds)
      },
      handleAppend () {
        this.checkedTarget = []
        this.taskInfo = {
      ...
      handleUpdate (data) {
        this.initSelect(data.orgTypeA.id)
        this.handleDetail(data)
        this.taskInfo = this.clone(data)
        this.checkedTarget = data.indIds
        ...
        this.$refs.updateForm.validate(valid => {
          if (valid) {
            ...
            new Promise((resolve, reject) => {
              ...
              this.getNodeTree(this.taskInfo)
            }).catch(() => {
              this.updateLoading = false
            })
          }
