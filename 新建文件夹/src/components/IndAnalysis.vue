          ...
          <div class="swiper-slide">
            ...
            <div class="main-box">
              <div style="overflow: hidden">
                ...
                <span class="year_text">数据年份&nbsp;</span>
                <DatePicker
                  :clearable="false"
                  v-model="selectYear"
                  type="year"
                ...
                <span class="year_text">教师年份&nbsp;</span>
                <DatePicker
                  :clearable="false"
                  v-model="selectTeacherYear"
                  type="year"
                  style="width: 100px;"
                  size="small"
                  placeholder="选择年"
                  @change="changeTeacherYear"
                  value-format="yyyy">
                </DatePicker>
              </div>
              <div class="graph-box">
          ...
          <div class="swiper-slide" v-for="(tds, index) in pages" :key="index">
            <table class="table-container" cellspacing="0" cellpadding="0">
              <tr class="table-header">
                <th :width="th.width" v-for="(th, thIndex) in ths" :key="th.id" :class="{'col-rank-label': !th.id}">
...
//        标识上一排序字段
    ...
    methods: {
      ...
      downCurrentData () {
        if (this.currentInd) {
          this.exportDataA({'name': '指标分析', 'url': 'v1/ind/' + this.currentInd + '/excdata?year=' + this.selectYear + '&teacher_year=' + this.selectTeacherYear})
        }
      },
      checkedChange (val) {
      ...
      changeYear () {
        this.getAnalysisData({'id': this.currentInd, 'year': Number(this.selectYear), 'teacher_year': Number(this.selectTeacherYear)})
      },
      changeTeacherYear () {
        this.getAnalysisData({'id': this.currentInd, 'year': Number(this.selectYear), 'teacher_year': Number(this.selectTeacherYear)})
      },
      ...
      setTds () {
        ...
        this.table.forEach((item, index) => {
          tds.push([item.org_name + '-' + item.org_pinyin, item.ind_val, item.org_rank, this.selectYear, this.selectTeacherYear, item.teacher_data[0].teacher, item.teacher_data[0].teacher_avg, item.teacher_data[0].teacher_avg_rank])
        })
        this.tds = tds
      },
...
//        高亮滑动项
    ...
    watch: {
      ...
      analysisData () {
        ...
        this.selectTeacherYear = this.currentInfo.teacher_year ? this.currentInfo.teacher_year + '' : ''
    ...
    mounted () {
      ...
      this.$nextTick(function () {
        ...
        this.getCheckedInds()
        this.getAnalysisData({'id': this.radios, 'year': -1, 'teacher_year': -1})
